...
for ME_RES_DIR in "$ME_RES_ROOT"/*; do
    ...
    sbatch scripts/ss_single_hyperparam_eval.sh $ME_SIM_INFO $ME_RES_DIR $ME_OUT_DIR
    sleep 0.1
done
...
for CO_RES_DIR in "$CO_RES_ROOT"/*; do
    ...
    sbatch scripts/ss_single_hyperparam_eval.sh $CO_SIM_INFO $CO_RES_DIR $CO_OUT_DIR
    sleep 0.1
done
